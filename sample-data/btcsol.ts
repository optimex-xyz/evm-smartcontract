...
//  @dev: In Solana, `pubkey` represents the user's address
//  thus, `toUserAddress` is the public key
//  example: `4VnhPS4Sc8xYLy9T9JZ7EBn2XxAm3Dpkw3mT7eEfzzVv`
export function getTradeInfo(
  ...
  const tradeInfo: ITypes.TradeInfoStruct = {
    ...
    toChain: [
      toUtf8Bytes(toUserAddress), // toUserAddress.
...
export function getRFQInfo(): ITypes.RFQInfoStruct {
  const rfqInfo: ITypes.RFQInfoStruct = {
    minAmountOut: parseUnits((300 * rand(1000)).toString(), 9), //  common `decimals` in Solana is 9
