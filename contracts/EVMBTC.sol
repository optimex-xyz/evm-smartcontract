...
******************************************************************************************/
contract EVMBTC is Core {
    /// Address of VaultRegistry contract
    IVaultRegistry public registry;

    constructor(IRouter router_, IVaultRegistry registry_) {
        router = router_;
        registry = registry_;
    }

    /**
        @notice Updates the address of the VaultRegistry contract.
        @dev Caller must be the `Owner` of the current Management contract.
        @param newRegistry The address of the new VaultRegistry contract.
    */
    function setVaultRegistry(
        address newRegistry
    ) external onlyManagementOwner notAddressZero(newRegistry) {
        registry = IVaultRegistry(newRegistry);
    }

    /**
        @notice Retrieves the contract type identifier.
        @return A string representing the contract type ("EVMBTC").
    */
    ...
    */
    function submitTrade(
        ...
        /// Verify mpcAssetPubkey and deposited `vaultAddress` validity
        ...
        if (
            registry.getVault(
                tradeData.tradeInfo.fromChain[1],
                tradeData.tradeInfo.fromChain[2]
            ) != address(bytes20(tradeData.scriptInfo.depositInfo[0]))
        ) revert VaultAddrNotMatched();

        {
            ...
            /// - The presign, structured with EIP-712, is also verified by the
            /// Vault contract (on the asset-chain) when MPC initiates the trade settlement
            bytes32 pmmId;
            for (uint256 i; i < presignList.length; i++) {
        ...
        /// Advance the trade stage and store data.
        /// @dev: For EVM->BTC, `pFeeRate` setting is snapshot, then
        /// `pFeeAmount` (protocol) and `aFeeAmount` (affiliate) are calculated
        uint256 pFeeRate = management.pFeeRate();
        uint256 pFeeAmount = (tradeData.tradeInfo.amountIn * pFeeRate) / DENOM;
        uint256 aFeeAmount = (tradeData.tradeInfo.amountIn *
            affiliateInfo.aggregatedValue) / DENOM;
        ...
        _feeDetails[tradeId] = FeeDetails({
            totalAmount: totalFeeAmount,
            pFeeAmount: pFeeAmount,
            aFeeAmount: aFeeAmount,
            pFeeRate: uint128(pFeeRate),
            aFeeRate: uint128(affiliateInfo.aggregatedValue)
        });
    ...
    */
    function selectPMM(
        ...
        /// Ensure `amountOut` >= `minAmountOut`
        /// @dev: For EVM->BTC, protocol fee is calculated when trade is submitted
        /// The PMM is already aware of the protocol fee and the actual trade amount (net amount after fees)
        /// As a result, this constraint differs from the one used in the BTCEVM contract
        if (info.pmmInfo.amountOut < info.rfqInfo.minAmountOut)
        ...
        /// Validate RFQ signature provided by User
    ...
    */
        ...
        isSuspended(STAGE.CONFIRM_SETTLEMENT)
    {
        ...
        address signer = _signer().getSettlementConfirmationSigner(
            tradeId,
            keccak256(abi.encode(totalFeeAmount, releaseTxId)),
            signature
        );
        _isValidMPCSignature(
            signer,
